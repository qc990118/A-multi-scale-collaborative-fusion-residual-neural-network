...
path = r"data/Dataset_A/1.mat"  # mat文件路径
...
class Data(object):
    ...
    def get_data(self):
        ...
        print(x)
        for i in range(len(file_list)):
            file_path = './dataseu/30_2/{}'.format(file_list[i])
...
# #
# hyj_model = DoubleChannelNet().to(device)
# hyj_model.load_state_dict(torch.load('weight/DoubleChannelNet-SEU-60--4.pt'))
# hyj_model.eval()
# activation = {}
#
...
# hyj_model.fc.register_forward_hook(get_activation('yyy'))
# test_features = []
# test_labels = []
# i = 0
...
# plot_tsne(test_features_2d, test_labels, colors, markers, sizes,"t-SNE of MCFRNN")
#
# time.sleep(3)
# a = 666
# from hhhh import MA1DCNN,MIXCNN,DCABiGRU,EWSNET,DoubleChannelNet,QCNN
#
# import torch
# from sklearn.manifold import TSNE
# # 导入您的模型
...
# # 加载模型和权重
# device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
# hyj_model = QCNN().to(device)
# hyj_model.load_state_dict(torch.load('weight/QCNN-SEU-50--4.pt'))
# hyj_model.eval()
#
# # 准备测试数据
# test_features = []
...
#     label = label.to(device, dtype=torch.long)
#     # 获取最后一层输出
#     last_layer_output = get_layer_output(hyj_model, img, "fc2")
#     last_layer_output = last_layer_output.view(last_layer_output.size(0), -1)
#     test_features.append(last_layer_output.cpu().detach().numpy())
#     test_labels.append(label.cpu().detach().numpy())
#
# test_features = np.concatenate(test_features, axis=0)
# test_labels = np.concatenate(test_labels, axis=0)
...
#
# # 调用 plot_tsne 函数并传入测试集的特征和标签
# plot_tsne(test_features_2d, test_labels, colors, markers, sizes, "t-SNE of EWSNet")
