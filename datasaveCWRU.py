...
raw_num = [25, 50, 75, 100]
load=["A","B","C","D"]
for load1 in load:
    for snr in snr_values:
        for snr in snr_values:
            print(f"Running experiment for load={load}and  raw_num={raw_num}and SNR={snr} ")
            ...
            class Data(object):
                ...
                def file_list(self):
                    return os.listdir('./data/Dataset_load1')
                ...
                def get_data(self):
                    ...
                    for i in range(len(file_list)):
                        file = scio.loadmat('./data/Dataset_load1/{}'.format(file_list[i]))
                        for k in file.keys():
...
# #
# hyj_model = DoubleChannelNet().to(device)
# hyj_model.load_state_dict(torch.load('weight/MCFRNNCWRU.pt'))
# hyj_model.eval()
# activation = {}
#
...
# plot_tsne(test_features_2d, test_labels, colors, markers, sizes,title="t-SNE of central fusion layer")
# time.sleep(3)
# a=666
