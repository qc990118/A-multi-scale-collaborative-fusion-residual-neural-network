...
from datasave2 import train_loader, test_loader, X_test
...
from AdamP_amsgrad import AdamP, SGDP, AdamW
...
# from AdamP_amsgrad import AdamP,SGDP,Adam
# optimizer = AdamP(model.parameters(), lr=0.001, weight_decay=0.0001, nesterov=True, amsgrad=True)
...
torch.save(model.state_dict(), 'weight/DoubleChannelNet1-OLSR-SGDP-bn-32-5.pt')
...
def get_layer_output(model, input_tensor, layer_name):
    """
    Get the output tensor of a specified layer in a model.
    Args:
        model: The neural network model.
        input_tensor: The input tensor to the model.
        layer_name: The name of the specified layer.
    Returns:
        output_tensor: The output tensor of the specified layer.
    """
    layer_output = None
    ...
    if layer_output is None:
        raise ValueError('No layer named {} in the model.'.format(layer_name))
    return layer_output
colors = ['#e38c7a', '#656667', '#99a4bc', 'cyan', 'blue', 'lime', 'r', 'violet', 'm', 'peru', 'olivedrab',
          'hotpink']
